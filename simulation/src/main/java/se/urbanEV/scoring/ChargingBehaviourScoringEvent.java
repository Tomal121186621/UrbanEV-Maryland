...
public class ChargingBehaviourScoringEvent extends Event implements HasPersonId {
    ...
    // MD fork (2026): effective session charging power in kW. Populated only by
    // the cost-bearing (costOnly=true) emission path so the scoring code can
    // turn delivered energy into dwell-time and apply the time-utility term.
    ...
    public ChargingBehaviourScoringEvent(double time,
                                         ...
                                         String activityType,
                                         double startSoc) {
        this(time, personId, soc, walkingDistance, activityType, startSoc, null, null, null, false, null);
    }
    ...
    public ChargingBehaviourScoringEvent(double time,
        ...
        this(time, personId, soc, walkingDistance, activityType, startSoc,
                pricingTime, energyChargedKWh, chargerType, costOnly, null);
    }

    // MD fork (2026): constructor overload with effective charging power.
    public ChargingBehaviourScoringEvent(double time,
                                         Id<Person> personId,
                                         Double soc,
                                         Double walkingDistance,
    ...
    public Double getEffectiveChargingPowerKW() { return effectiveChargingPowerKW; }
    ...
    public Map<String, String> getAttributes() {
        ...
        if (effectiveChargingPowerKW != null) {
            attributes.put("effectiveChargingPowerKW", effectiveChargingPowerKW.toString());
        }
