...
public class DriveDischargingHandler
	...
	// MD fork (2026-07): PHEV gas fallback. Battery.changeSoc clamps at 0, silently
	// discarding demanded energy the battery cannot supply — physically, those miles are
	// driven in charge-sustaining (gasoline) mode. Track the clamped deficit per vehicle
	// and emit it as a "gas_fallback" scoring event at the end of each leg, so
	// ChargingBehaviourScoring can price it (PHEVs) or ignore it (BEV telemetry).
	private final Map<Id<Vehicle>, Double> deficitJ = new HashMap<>();
	private final org.matsim.core.api.experimental.events.EventsManager eventsManager;

	@Inject
	public DriveDischargingHandler(ElectricFleet data, Network network, EvConfigGroup evCfg,
                                   MobsimScopeEventHandling events,
                                   org.matsim.core.api.experimental.events.EventsManager eventsManager) {
	...
	public void handleEvent(VehicleLeavesTrafficEvent event) {
		...
		if (evDrive != null) {
			...
			// MD fork (2026-07): flush accumulated battery deficit for this leg as a
			// gas_fallback scoring event (person id == vehicle id by convention).
			Double dJ = deficitJ.remove(evDrive.vehicleId);
			if (dJ != null && dJ > 0) {
				double deficitKWh = dJ / 3_600_000.0;
				eventsManager.processEvent(new se.urbanEV.scoring.ChargingBehaviourScoringEvent(
						event.getTime(),
						Id.createPersonId(evDrive.vehicleId.toString()),
						0.0, 0.0,
						"gas_fallback",
						0.0,
						null, deficitKWh, null, false));
			}
		}
	}
	...
	private EvDrive dischargeVehicle(Id<Vehicle> vehicleId, Id<Link> linkId, double eventTime) {
		...
		if (evDrive != null && !evDrive.isOnFirstLink()) {// handle only our EVs, except for the first link
			...
			// MD fork (2026-07): record the portion of demanded energy the battery cannot
			// supply (changeSoc clamps at 0) — those link-meters are driven on gasoline
			// for PHEVs. Accumulated per leg; flushed in handleEvent(VehicleLeavesTrafficEvent).
			if (energy > 0) {
				double available = ev.getBattery().getSoc();
				double clamped = Math.max(0.0, energy - available);
				if (clamped > 0) {
					deficitJ.merge(vehicleId, clamped, Double::sum);
				}
			}
