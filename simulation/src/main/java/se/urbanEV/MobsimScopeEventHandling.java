...
public class MobsimScopeEventHandling implements StartupListener, AfterMobsimListener {
	...
	private void addPrivateCharger(Person person, String activityType, double power) {
		...
		// MD fix: tag private chargers with their activity type ("home"/"work") instead
		// of "default". This makes them findable by the 5-way occupancy collector AND
		// makes ChargingBehaviourScoring's 5-way switch route them to homeChargingCost /
		// workChargingCost. Paired with the allowlist-bypass in
		// VehicleChargingHandler.findBestCharger (line ~491) so EV.charger_types
		// (L1/L2/DCFC[/DCFC_TESLA]) does not need to include "home"/"work".
		String chargerType = activityType;
