...
public class SmartChargingScheduler {
    ...
    public synchronized void processDueTasks(double now) {
        ...
        // Defer re-inserts (charger-full retries) until after iteration completes
        // to avoid ConcurrentModificationException on the entrySet iterator.
        java.util.List<ScheduledCharge> retries = null;
            ...
            if (sc.startTime <= now + 1e-3) {
                ...
                if (plugged >= plugs) {
                    log.warn("SmartChargingScheduler: charger FULL at plug time; ev=" + sc.evId
                            ...
                            + " -> retry in 300s");
                    if (retries == null) retries = new java.util.ArrayList<>();
        ...
        if (retries != null) {
            for (ScheduledCharge r : retries) {
                scheduled.put(r.evId, r);
            }
        }
    }
